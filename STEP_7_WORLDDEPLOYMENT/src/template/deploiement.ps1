...
$planname="cnaplanL1"+$suffixe
$webappname="cnawebuiL1"+$suffixe

$location2="eastus2"
$plannameLoc2="cnaplanL2"+$suffixe
$webappnameLoc2="cnawebuiL2"+$suffixe

$location3="westeurope"
$plannameLoc3="cnaplanL3"+$suffixe
$webappnameLoc3="cnawebuiL3"+$suffixe
...
####################################################################################
# Compilation des applications ( REUTILISATION DU CODE DU STEP_1 !! )
Remove-Item WebAppUI.zip  
dotnet publish ..\..\..\STEP_1_WW_COSMOSDB\src\WebAppUI\CnAppForAzureDev.csproj
Compress-Archive -Path ..\..\..\STEP_1_WW_COSMOSDB\src\WebAppUI\bin\Debug\netcoreapp3.1\publish\* -DestinationPath WebAppUI.zip  
...
az webapp config appsettings set -g $rgname -n $webappname --settings UseCosmosDb=true AccountKey=$accountkey AccountName=$accountname CosmosDbConnectionString=$cosmosdbconnectionstring CatalogName=$tablename ContainerName=$containername  MaxItems=25 MaxItemsOnHomePage=6 ApplicationInsightsAgent_EXTENSION_VERSION="~2" 
...
az webapp deployment source config-zip -g $rgname -n $webappnameLoc2 --src .\WebAppUI.zip
az webapp stop -g $rgname -n $webappnameLoc2
az webapp start -g $rgname -n $webappnameLoc2

az webapp config appsettings set -g $rgname -n $webappnameLoc2 --settings UseCosmosDb=true AccountKey=$accountkey AccountName=$accountname CosmosDbConnectionString=$cosmosdbconnectionstring CatalogName=$tablename ContainerName=$containername  MaxItems=25 MaxItemsOnHomePage=6 ApplicationInsightsAgent_EXTENSION_VERSION="~2" 
...
az webapp deployment source config-zip -g $rgname -n $webappnameLoc3 --src .\WebAppUI.zip
az webapp stop -g $rgname -n $webappnameLoc3
az webapp start -g $rgname -n $webappnameLoc3

az webapp config appsettings set -g $rgname -n $webappnameLoc3 --settings UseCosmosDb=true AccountKey=$accountkey AccountName=$accountname CosmosDbConnectionString=$cosmosdbconnectionstring CatalogName=$tablename ContainerName=$containername  MaxItems=25 MaxItemsOnHomePage=6 ApplicationInsightsAgent_EXTENSION_VERSION="~2" 
